...
void	sort_v2(t_stack *stack_a, t_stack *stack_b, int step)
{
	...
	if (dll_has_higher(stack_a->front, step))
		sort_v2(stack_a, stack_b, step + 30);
}
		...
		t_moves *moves, int step)
{
	...
	if (b->front && a->front)
	{
		target_b = dll_next_lower_front(b->front, target_a);
		fp_printf(" target_b: %i\n", target_b);
		if (dll_next_lower(b->front, target_a) <= (dll_size(b->front) * .5))
			moves->rb = dll_index(b->front, target_b) - 1;
		else if (dll_size(b->front) > 2)
			moves->rrb = dll_rindex(b->front, target_b);
	}
	if (a->front)
	{
		if (dll_index(a->front, target_a) < dll_size(a->front) * .5)
